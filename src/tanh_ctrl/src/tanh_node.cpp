...
void TanhNode::createRosInterfaces() {
  ...
  torque_sp_pub_ = this->create_publisher<px4_msgs::msg::VehicleTorqueSetpoint>("/fmu/in/vehicle_torque_setpoint", qos_default);
...
void TanhNode::attitudeControlLoop(uint64_t sample_us) {
  ...
  const ControlOutput publish_out = gateControlOutputForPublicationState(out, is_armed_, is_offboard_);
...
void TanhNode::publishWrenchSetpoint(const ControlOutput& out, uint64_t now_us, uint64_t sample_us) {
